...
export function PageIntro({
  ...
  return (
    <section className="site-container px-4 md:px-6">
      <div
        data-page-intro={compact ? "compact" : "default"}
        className={cn(
          "panel-strong relative overflow-hidden rounded-[2rem] px-5 py-7 min-[360px]:px-6 md:px-10",
          compact ? "md:py-8 lg:mx-auto lg:max-w-[68rem]" : "md:py-10",
          !media && !compact && "lg:mx-auto lg:max-w-[74rem]"
        )}
      >
        ...
        <div
          className={cn(
            "relative",
            media &&
              "grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.78fr)] lg:gap-12",
            media && mediaAlignment === "center" ? "lg:items-center" : "lg:items-start"
          )}
        >
          <div className="max-w-3xl space-y-4 min-[480px]:space-y-5">
            ...
            <h1
              className={cn(
                "text-[2.35rem] font-semibold leading-[1.02] text-primary min-[360px]:text-[2.65rem] min-[480px]:text-5xl",
                compact ? "sm:text-[3.5rem]" : "sm:text-6xl"
              )}
            >
              {title}
            </h1>
            ...
            {actions ? (
              <div className="grid grid-cols-1 gap-2.5 pt-1 [&>*]:w-full [&>*]:justify-center min-[480px]:grid-cols-2 min-[480px]:[&>*:only-child]:col-span-2 sm:flex sm:flex-wrap sm:gap-3 sm:[&>*]:w-auto">
                {actions}
              </div>
            ) : null}
          </div>
