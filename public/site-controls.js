(() => {
  ...
  document.querySelectorAll("[data-theme-toggle]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      try {
        localStorage.setItem("biromd-theme", dark ? "dark" : "light")
      } catch {}
      syncThemeControls()
    })
  })
  ...
  const footerLayout = window.matchMedia("(min-width: 768px)")
  const syncFooterDisclosures = () => {
    document.querySelectorAll(".footer-disclosure").forEach((details) => {
      details.open = footerLayout.matches
    })
  }
  footerLayout.addEventListener?.("change", syncFooterDisclosures)
  ...
  const hydrateClinicalImages = (container) => {
    ...
    container?.querySelectorAll("[data-clinical-src]").forEach((image) => {
